...
__global__ void check_d_topo_result_gpu(int* d_topo_result_gpu, int num_nodes, uint32_t* write_size) {
...
void Timer::call_cuda_topo_centric_vector() {
  ...
  checkError(cudaMalloc(&write_size, sizeof(uint32_t)), "write_size allocation failed");
  ...
  checkError(cudaMemset(write_size, 0, sizeof(uint32_t)), "write_size memset failed");
  ...
  while(read_size > 0) { 
    ...
    check_d_topo_result_gpu<<<1, 1>>>(d_topo_result_gpu, read_offset + read_size, write_size);

    // calculate where to read for next iteration
    read_offset += read_size;
    checkError(cudaMemcpy(&read_size, write_size, sizeof(uint32_t), cudaMemcpyDeviceToHost), "queue_size memcpy failed");

    // set write_size = 0 for next iteration 
    checkError(cudaMemset(write_size, 0, sizeof(uint32_t)), "write_size rewrite failed");
