...
__global__ void check_result_gpu(
  ...
  long long* d_topo_fu_partition_result_gpu_8_btyes,
  int* d_seg_heads,
  int* d_reduce_value,
  ...
  if(tid == 0) {
    ...
    printf("\n");
    printf("after pushed, d_topo_fu_partition_result_gpu_8_btyes(threadId = %d) = ", tid);
    for(int i=0; i<num_nodes; i++) {
      printf("%lld ", d_topo_fu_partition_result_gpu_8_btyes[i]);
    }
    printf("\n");
...
__global__ void combineToIntsKernel(int* high, int* low, long long* result, int read_offset, int read_size) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < read_size) {
      result[idx] = (long long) high[read_offset + idx] << 32; 
      result[idx] += low[read_offset + idx]; 
    }
}

__global__ void reassign_d_topo_fu_partition_result_gpu(
  int* d_topo_result_gpu,
  ...
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx < read_size) {
      d_topo_fu_partition_result_gpu[read_offset + idx] = d_fu_partition[d_topo_result_gpu[read_offset + idx]];
  }
}
...
__global__ void decide_partition_id( // decide partition id for each node 
  ...
  if(tid < read_size) {
    ...
    // check how many nodes can still be merged in this partition
    int left_over = partition_size - d_partition_counter_gpu[d_fu_partition[d_topo_result_gpu[read_offset + tid]]]; 
...
__global__ void assign_partition_id(
  ...
  if(tid < read_size) {
    ...
    if(d_new_partition[tid_index] == 1) {
      ...
      d_partition_result_gpu[cur_id] = d_fu_partition[d_topo_result_gpu[read_offset + tid]];
      atomicAdd(&d_partition_counter_gpu[d_fu_partition[d_topo_result_gpu[read_offset + tid]]], 1);
    }
  }
}
...
void Timer::call_cuda_partition_deterministic() {
  ...
  checkError_t(cudaMalloc(&d_topo_fu_partition_result_gpu_8_btyes, sizeof(long long)*num_nodes), "d_topo_fu_partition_result_gpu_8_btyes allocation failed");
  checkError_t(cudaMalloc(&d_partition_result_gpu, sizeof(int)*num_nodes), "d_partition_result_gpu allocation failed");
  checkError_t(cudaMalloc(&d_partition_counter_gpu, sizeof(int)*num_nodes), "d_partition_counter_gpu allocation failed");
  checkError_t(cudaMalloc(&write_size, sizeof(uint32_t)), "write_size allocation failed");
  checkError_t(cudaMalloc(&max_partition_id, sizeof(int)), "max_partition_id allocation failed");
  ...
  while(read_size > 0) { 
    ...
    assign_partition_id<<<num_block, BLOCK_SIZE>>>(
      ...
      d_fu_partition, // the future partition this node will be assigned to(if partition not full)
      d_partition_result_gpu,
      d_partition_counter_gpu,
      max_partition_id,
    ...
    //   d_topo_fu_partition_result_gpu_8_btyes,
    //   d_seg_heads,
    //   d_reduce_value,
    //   d_incre_max_partition_id, // the incremental value to max_partition_id for each node  
    //   d_new_partition, // decide whether this node should apply d_incre_max_partition_id to get new partition id
    ...
    combineToIntsKernel<<<num_block, BLOCK_SIZE>>>(
      d_topo_fu_partition_result_gpu, 
      d_topo_result_gpu, 
      d_topo_fu_partition_result_gpu_8_btyes, 
      read_offset, read_size
    ...
    mgpu::mergesort(d_topo_fu_partition_result_gpu_8_btyes, d_topo_result_gpu+read_offset, read_size, []MGPU_DEVICE(long long a, long long b){return a < b;}, context);
    ...
    reassign_d_topo_fu_partition_result_gpu<<<num_block, BLOCK_SIZE>>>(
      d_topo_result_gpu,
      d_topo_fu_partition_result_gpu, // future partition result stored in topoligical order
      read_offset, read_size,
      d_fu_partition // the future partition this node will be assigned to(if partition not full)
  ...
  checkError_t(cudaFree(d_topo_fu_partition_result_gpu_8_btyes), "d_topo_fu_partition_result_gpu_8_btyes free failed");
  checkError_t(cudaFree(d_partition_result_gpu), "d_partition_result_gpu free failed");
  checkError_t(cudaFree(d_partition_counter_gpu), "d_partition_counter_gpu free failed");
  checkError_t(cudaFree(write_size), "write_size free failed");
  checkError_t(cudaFree(max_partition_id), "max_partition_id free failed");
