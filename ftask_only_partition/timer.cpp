...
//  _build_prop_tasks();
  ...
  // initialize vivekDAG only for ftask
  ...
  _vivek_ftask_rebuild_time += std::chrono::duration_cast<std::chrono::microseconds>(end-start).count();

  // Execute the ftask
  start = std::chrono::steady_clock::now();
  _executor.run(_taskflow).wait();
  end = std::chrono::steady_clock::now();
  _vivek_ftask_runtime += std::chrono::duration_cast<std::chrono::microseconds>(end-start).count();
...
void Timer::_initialize_local_crit_cost_pins() {
   ...
   * use Depth-First Search (DFS) with topological sorting to
...
void Timer::_initialize_vivekDAG() {
   ...
   * in this implementation, we only do partition for ftask
   */

  std::cerr << "initializing vivekDAG for only ftasks.\n";
