...
#include <ot/taro/taro/scheduler/events.hpp>
...
void Timer::_update_timing() {
  ...
  // use taro to run ftask part
  _run_GDCA_ftask_taro();  
  // _run_GDCA_ftask_taro_event_trigger();
  
  // rebuild btask part of _taskflow
  _rebuild_btask(); 
...
void Timer::_initialize_vivekDAG() {
  ...
  // add tasks to global task vector
  for(size_t i=0; i<_vivekDAG._vtask_ptrs.size(); i++) {
  ...
  // _fprop_cands(_vtask_ptrs) stores the topological order of all the vtasks
  for(auto pin : _fprop_cands) {
...
void Timer::_rebuild_taskflow_vivek_btask() {
...
void Timer::_rebuild_taskflow_GDCA_btask() {
...
size_t Timer::_fill_events_taro() {

  /*
   * traverse _vivekDAG, check _fanout for each node, if there are fanout nodes that does not belong to its parent's partition
  ...
  for(auto task : _vivekDAG._vtask_ptrs) {
    for(auto successor_id : task->_fanout) {
      if(_vivekDAG._vtask_ptrs[successor_id]->_cluster_id != task->_cluster_id) { // if this successor does not belong to 
                                                                                  // the same partition as its parent
        task->_pins[0].second->_set_events.push_back(event); // set an event for parent
        _vivekDAG._vtask_ptrs[successor_id]->_pins[0].second->_wait_events.push_back(event); // wait this event for child 
        event ++; // different child has its own event, multiple children wait for same event is not doable
      }
    } 
  }
...
void Timer::_run_GDCA_ftask_taro_event_trigger() {

  /*
  // build DAG using taskflow to debug
  // clear original taskflow
  _taskflow.clear();
  ...
  */
  ...
  size_t num_events = _fill_events_taro();

  taro::Taro taro{std::thread::hardware_concurrency()};
  auto events = taro.event_scheduler(num_events);

  // emplace all tasks in vivekDAG to _taskflow
  for(auto task : _rebuild_vivekDAG._vtask_ptrs) {
    task->_tarotask = taro.emplace([this, task, &events] () -> taro::Coro {
      auto start = std::chrono::steady_clock::now();
      for(auto pair : task->_pins) {
        if(pair.first) {
          /*
           * first check is there any event it needs to wait
           */
          for(auto wait_event : pair.second->_wait_events) {
            co_await events.until(wait_event);
          }
          /*
           * then do its task
           */
          ...
          _fprop_test(*(pair.second));
          /*
           * last set events for its children
           */
          for(auto set_event : pair.second->_set_events) {
            events.set(set_event);
          }
        }
        else {
          _bprop_rat(*(pair.second));
  ...
  taro.wait();
  auto end = std::chrono::steady_clock::now();
  GDCA_taro_runtime += std::chrono::duration_cast<std::chrono::microseconds>(end-start).count();
}

void Timer::_run_GDCA_ftask_taro() {
  // clear original taskflow
  _taskflow.clear();
  ...
  // emplace all tasks in vivekDAG to _taskflow
  for(auto task : _rebuild_vivekDAG._vtask_ptrs) {
    task->_tarotask = taro.emplace([this, task] () {
      auto start = std::chrono::steady_clock::now();
      for(auto pair : task->_pins) {
        if(pair.first) {    
          _fprop_rc_timing(*(pair.second));
  ...
  for(size_t task_id=0; task_id<_rebuild_vivekDAG._vtask_ptrs.size(); task_id++) {
    for(auto successor_id : _rebuild_vivekDAG._vtask_ptrs[task_id]->_fanout) {
        _rebuild_vivekDAG._vtask_ptrs[task_id]->_tarotask.precede(_rebuild_vivekDAG._vtask_ptrs[successor_id]->_tarotask);
    }
  }
...
void Timer::_run_vivekDAG_GDCA_seq_btask() {
...
//  std::cerr << _vivekDAG._vtask_ptrs.size() << "\n";
  ...
  for(auto pin : _frontiers) {
    ...
    pin->_fvid = 0;
    pin->_bvid = 0;
    pin->_set_events.clear();
    pin->_wait_events.clear();
  }
}
