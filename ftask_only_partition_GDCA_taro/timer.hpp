...
class Timer {
  ...
  public:
    ...
    ~Timer() {
      ...
      std::cout << "taro ftask runtime(after partition) : " << GDCA_taro_runtime << "\n";
  ...
  private:
    ...
    // taro 
    // taro::Taro _taro{std::thread::hardware_concurrency()};
    ...
    void _rebuild_taskflow_vivek_btask();
    void _rebuild_taskflow_GDCA_btask();
    void _run_GDCA_ftask_taro();
    void _run_GDCA_ftask_taro_event_trigger();
    void _run_vivekDAG_GDCA_seq_btask(); // run GDCA partitioned vivekDAG sequentially

    // traverse _vivekDAG to fill _set_events and _wait_events for each node
    // return how many events are set
    size_t _fill_events_taro(); 
