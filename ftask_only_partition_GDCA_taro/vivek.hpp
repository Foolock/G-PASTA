...
namespace ot {
  ...
  class VivekTask {
    ...
    public:
      ...
      void _add_set_event(size_t event) {
        _set_events.push_back(event);
      }

      void _add_wait_event(size_t event) {
        _wait_events.push_back(event);
      }
    ...
    private:
      ...
      bool _pushed = false;
      std::vector<size_t> _set_events;
      std::vector<size_t> _wait_events;
  };
