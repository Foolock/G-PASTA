...
//  _build_prop_cands();
  ...
  for(auto pin : _fprop_cands) {
    ...
    pin->_ftask = _taskflow.emplace([this, pin] () {
      ...
      _fprop_at(*pin);
      _fprop_test(*pin);
    }).name(std::to_string(pin->_fvid)+'('+std::to_string(pin->_flocal_crit_cost)+')');
  }
  ...
  for(auto pin : _bprop_cands) {
    ...
    pin->_btask = _taskflow.emplace([this, pin] () {
      _bprop_rat(*pin);
    }).name(std::to_string(pin->_bvid)+'('+std::to_string(pin->_blocal_crit_cost)+')');
  }
...
//  }
  ...
  _build_prop_cands_time += std::chrono::duration_cast<std::chrono::microseconds>(end-start).count();

//  std::cout << "total tasks amount: " << _frontiers.size() * 2 << "\n";

  _it_update_timing++;   
//  std::cerr << _it_update_timing << " update_timing\n";

//  std::cerr << "runnning vivek partition.\n";
  /*
   * vivek's implementation
   */
  start = std::chrono::steady_clock::now();
  _initialize_vivekDAG(); 
//  std::cerr << "after initialization, vivekDAG size: " << _vivekDAG._vtask_ptrs.size() << "\n"; 
  end = std::chrono::steady_clock::now();
  _initialize_DAG += std::chrono::duration_cast<std::chrono::microseconds>(end-start).count();

  _build_prop_tasks();
//  _taskflow.dump(std::cout);

  start = std::chrono::steady_clock::now();
  _partition_vivekDAG_vector();
  end = std::chrono::steady_clock::now();
  _partition_DAG += std::chrono::duration_cast<std::chrono::microseconds>(end-start).count();

  start = std::chrono::steady_clock::now();
  _rebuild_taskflow();
  end = std::chrono::steady_clock::now();
  _mapping_taskflow += std::chrono::duration_cast<std::chrono::microseconds>(end-start).count();
...
//  _taskflow.dump(std::cout);
  ...
  _taskflow_runtime += std::chrono::duration_cast<std::chrono::microseconds>(end-start).count();
  _taskflow.clear();
...
void Timer::_initialize_vivekDAG() {
  ...
  for(auto pin : _fprop_cands) {
    _vivekDAG.addVivekTask(pin->_fvid, pin->_flocal_crit_cost, std::make_pair(true, pin)); 
  }
  ...
  for(auto pin : _fprop_cands) {
    int cur_vtask = pin->_fvid;
    for(auto arc : pin->_fanin) {
      Pin* from = &(arc->_from);
      int from_id = from->_fvid;
      _vivekDAG._vtask_ptrs[cur_vtask]->addFanin(from_id); 
    }
    for(auto arc : pin->_fanout) {
      Pin* to = &(arc->_to);
      int to_id = to->_fvid;
      _vivekDAG._vtask_ptrs[cur_vtask]->addFanout(to_id); 
    }
  }
  ...
  for(auto pin : _fprop_cands) {
    if(pin->_fanout.size() == 0) {
      int cur_vftask = pin->_fvid;
      int cur_vbtask = pin->_bvid;
      _vivekDAG._vtask_ptrs[cur_vftask]->addFanout(pin->_bvid); 
      _vivekDAG._vtask_ptrs[cur_vbtask]->addFanin(pin->_fvid); 
    }
  }
  ...
  for(size_t i=0; i<_vivekDAG._vtask_ptrs.size(); i++) {
    _global_task_queue.push(_vivekDAG._vtask_ptrs[i]);
  }
  for(size_t i=0; i<_vivekDAG._vtask_ptrs.size(); i++) {
  ...
  /* 
  for(auto pin : _fprop_cands) {
    _top_down_topo_order_cur_vivekDAG_vector.push_back(pin->_fvid); 
  }
  for(auto pin : _bprop_cands) {
  ...
  */
  for(auto task : _vivekDAG._vtask_ptrs) {
    _top_down_topo_order_cur_vivekDAG_vector.push_back(task->_id);
  }
  /*
...
void Timer::_partition_vivekDAG_vector() {

  size_t num_merge = 100;  
  size_t merge_cnt = 0;
  size_t cur_task = 0;
  size_t original_size_vivekDAG = _vivekDAG._vtasks.size();
  while(1) {
    ...
    // print cost
    // std::cerr << "id: " << least_cost_task->_id << " cost: " << least_cost_task->_local_crit_cost << "\n"; 

    // get dependent and successor tasks of this task and store into local queue
    // std::priority_queue<VivekTask*, std::vector<VivekTask*>, CompareTaskByCost> local_task_queue;
    ...
    std::sort(local_task_vector.begin(), local_task_vector.end(), [](const VivekTask* a, VivekTask* b){
      return a->_self_cost < b->_self_cost;
    });
    ...
    while(1) {
      ...
      if(local_task_vector.size() == 0) {
        break; 
      }

      if(cur_neighbor > local_task_vector.size() - 1) {
        break;
      } 
      ...
      // after merged, update _top_down_topo_order_cur_vivekDAG_vector for _update_local_crit_cost()
      ...
      auto end = std::chrono::steady_clock::now();
      _update_local_crit_cost_time += std::chrono::duration_cast<std::chrono::microseconds>(end-start).count();
      // _print_updated_cost();

      // push back the merged task to global task queue
      _global_task_vector.push_back(_vivekDAG._vtask_ptrs[_vivekDAG._vtask_ptrs.size()-1]);

      // sort the _global_task_vector
      start = std::chrono::steady_clock::now();
      std::sort(_global_task_vector.begin(), _global_task_vector.end(), [](const VivekTask* a, VivekTask* b){ 
        return a->_local_crit_cost < b->_local_crit_cost;
      });
      end = std::chrono::steady_clock::now();
      _sorting_time += std::chrono::duration_cast<std::chrono::microseconds>(end-start).count();
...
void Timer::_update_local_crit_cost() {
  ...
  */
  ...
  auto end = std::chrono::steady_clock::now();
  _dfs_topo_order_time += std::chrono::duration_cast<std::chrono::microseconds>(end-start).count();
  ...
  */
  ...
  _top_down_update_time += std::chrono::duration_cast<std::chrono::microseconds>(end-start).count();
  ...
  */
  ...
  _bottom_up_update_time += std::chrono::duration_cast<std::chrono::microseconds>(end-start).count();
...
void Timer::_rebuild_taskflow() {
  ...
  for(auto task : _vivekDAG._vtask_ptrs) {
    if(!task->_merged) {
      ...
      }).name(std::to_string(task->_id)+'('+std::to_string(task->_local_crit_cost)+')');  
    }
  }

  // build dependencies for all tasks emplaced in _taskflow
  /*
  while(!_top_down_topo_order_cur_vivekDAG.empty()) {
    int task_id = _top_down_topo_order_cur_vivekDAG.top();
    _top_down_topo_order_cur_vivekDAG.pop();
...
void Timer::_print_updated_cost() {

  std::cerr << "-------------------local critical path cost after update-------------------\n";
  for(auto task : _vivekDAG._vtask_ptrs) {
    std::cerr << "task " << task->_id << ": " << task->_local_crit_cost << "\n";
    std::cerr << "fanin: ";
    for(auto id : task->_fanin) {
      std::cerr << id << " ";
    }
    std::cerr << "| fanout: ";
    for(auto id : task->_fanout) {
      std::cerr << id << " ";
    }
    std::cerr << "\n";
  }
