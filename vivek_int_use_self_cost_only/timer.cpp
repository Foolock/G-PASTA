...
//  std::cerr << "after initialization, vivekDAG size: " << _vivekDAG._vtask_ptrs.size() << "\n"; 
  ...
  start = std::chrono::steady_clock::now();
  _partition_vivekDAG_no_cost_update();
  end = std::chrono::steady_clock::now();
...
void Timer::_partition_vivekDAG_no_cost_update() {
  ...
  while(1) {
    ...
    if(cur_task > _vivekDAG._vtasks.size() - 1 || cur_task >= num_merge) {
      break;
    }
    ...
    while(1) {
      ...
      // std::cerr << "least_cost_task(" << least_cost_task->_id << "): " << least_cost_task->_self_cost << "\n";
      // std::cerr << "least_cost_neighbor(" << least_cost_neighbor->_id << "): " << least_cost_neighbor->_self_cost << "\n";
      // std::cerr << "merged_task(" << _vivekDAG._vtask_ptrs.size()-1 << "): " << _vivekDAG._vtask_ptrs[_vivekDAG._vtask_ptrs.size()-1]->_self_cost << "\n";
