...
class Timer {
  ...
  public:
    ...
    ~Timer() {
      ...
      std::cout << "_build_prop_cands_time: " << _build_prop_cands_time << "\n"; 
      std::cout << "_initialize_DAG runtime: " << _initialize_DAG << "\n"; 
      std::cout << "_partition_DAG runtime: " << _partition_DAG << "\n"; 
      std::cout << "_mapping_taskflow runtime: " << _mapping_taskflow << "\n"; 
      std::cout << "_update_local_crit_cost_time: " << _update_local_crit_cost_time << "\n"; 
      std::cout << "_dfs_topo_order_time: " << _dfs_topo_order_time << "\n"; 
      std::cout << "_top_down_update_time: " << _top_down_update_time << "\n"; 
      std::cout << "_bottom_up_update_time: " << _bottom_up_update_time << "\n"; 
      std::cout << "_sorting_time: " << _sorting_time << "\n"; 
      std::cout << "partitioned taskflow runtime: " << _taskflow_runtime << "\n"; 
  ...
  private:
    ...
    size_t _it_update_timing = 0; // a counter(iteration) for num of update_timing

    size_t _build_prop_cands_time = 0;

    size_t _initialize_DAG = 0;

    size_t _partition_DAG = 0;

    size_t _update_local_crit_cost_time = 0;

    size_t _dfs_topo_order_time = 0;

    size_t _top_down_update_time = 0;
    
    size_t _bottom_up_update_time = 0;

    size_t _sorting_time = 0;

    size_t _mapping_taskflow = 0;

    size_t _taskflow_runtime = 0;
    ...
    void _partition_vivekDAG_vector();
    void _partition_vivekDAG_no_cost_update();

    // print runtime for each task
    void _timing_profile();
